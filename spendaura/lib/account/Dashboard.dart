...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadExpenseChartData() async {
    if (userId == null) return;
    try {
      final token = await UserPreferences.getUserToken();
      final chart = await TransactionService.getExpenseCategoriesChart(userId!, token!);
      setState(() {
        chartImage = chart;
      });
    } catch (e) {
  ...
  Future<void> _loadIncomeChartData() async {
    if (userId == null) return;
    try {
      final token = await UserPreferences.getUserToken();
      final chart = await TransactionService.getIncomeCategoriesChart(userId!, token!);
      setState(() {
        chartImage = chart;
      });
    } catch (e) {
